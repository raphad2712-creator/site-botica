...
export function OrderAftercare({ pedidos, solicitacoes }: { pedidos: Pedido[]; solicitacoes: Solicitacao[] }) {
  ...
  if (!pedidos.length) return <div className="account-empty"><span>⌑</span><h3>Você ainda não fez nenhum pedido</h3><p>Explore nossos produtos e encontre o cuidado ideal para você.</p><Link href="/#produtos">VER PRODUTOS</Link></div>;

  return <div className="account-orders">{pedidos.map((pedido) => {
    ...
    return <article className="order-card" key={pedido.id}>
      ...
      <button className="aftercare-open" type="button" onClick={() => { setAberto(aberto === pedido.id ? null : pedido.id); setMensagem(""); }}>{aberto === pedido.id ? "FECHAR" : "SOLICITAR TROCA, DEVOLUÇÃO OU REEMBOLSO"}</button>
