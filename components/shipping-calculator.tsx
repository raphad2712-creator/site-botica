...
export function ShippingCalculator({ subtotal }: { subtotal: number }) {
  ...
  return <section className="product-shipping" aria-labelledby="shipping-title">
    ...
    {cotacao && <div className="product-shipping-result" role="status"><span><b>Entrega padrão para {cotacao.destino}</b><small>{cotacao.prazoMinimo} a {cotacao.prazoMaximo} dias úteis</small></span><strong>{cotacao.gratis ? "GRÁTIS" : moeda(cotacao.valor)}</strong></div>}
