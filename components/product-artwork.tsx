...
import { consultationArtwork, consultationDarkImage, consultationImage, type ConsultationProduct } from "@/lib/consultation-products";
  ...
  return <figure className="product-artwork" aria-label={`Fotos de ${produto.nome}`}>
    <div className="product-gallery-track" ref={track} tabIndex={0} aria-label="Galeria do produto; deslize para ver as informações"
      ...
      {sources.map((source, index) => <div className="product-gallery-slide" key={source}>
        ...
        <Image className={index === 0 ? "natural-image-light" : ""} src={source} alt={index === 0 ? `${produto.nome} ${produto.dose}, ${produto.apresentacao}, foto do produto` : `Informações de ${produto.nome} ${produto.dose}`} width={1254} height={1254} sizes="(max-width: 760px) 100vw, 650px" priority={index === 0} />
      </div>)}
    </div>
    <div className="product-gallery-controls">
      ...
      <div>{sources.map((source, index) => <button type="button" key={source} aria-label={index === 0 ? "Ver foto do produto" : "Ver imagem das informações"} aria-current={active === index ? "true" : undefined} onClick={() => show(index)}>{index === 0 && <Image className="natural-image-dark" src={consultationDarkImage(produto)} alt="" width={48} height={48} />}<Image className={index === 0 ? "natural-image-light" : ""} src={source} alt="" width={48} height={48} /></button>)}</div>
