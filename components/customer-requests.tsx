...
export function CustomerRequests({ solicitacoes, pedidos }: { solicitacoes: Solicitacao[]; pedidos: Pedido[] }) {
  if (!solicitacoes.length) return <div className="customer-requests-empty"><span><Icon name="document" /></span><h3>Nenhuma solicitação enviada</h3><p>Quando você solicitar uma troca, devolução ou reembolso, o acompanhamento aparecerá aqui.</p><a href="#pedidos">VER MEUS PEDIDOS</a></div>;

  return <div className="customer-request-list">{solicitacoes.map((item) => {
    const codigo = String(item.pedido_id).padStart(6, "0");
