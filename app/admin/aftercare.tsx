...
export function AdminAftercare({ pedidos, solicitacoes, receitasAprovadas }: { pedidos: PedidoAdmin[]; solicitacoes: SolicitacaoAdmin[]; receitasAprovadas: ReceitaAprovada[] }) {
  ...
  return <div className="admin-aftercare">
    ...
    <section id="entregas-admin"><div className="admin-panel-heading"><div><small>LOGÍSTICA</small><h2>Entregas e rastreamento</h2><p>Escolha a etapa, informe o código e o cliente receberá a atualização por e-mail.</p></div><span>{pedidosAtivos.length} PEDIDOS</span></div>
      <div className="admin-cards admin-delivery-cards">{pedidosAtivos.length ? pedidosAtivos.map((pedido) => { const codigo = String(pedido.id).padStart(6, "0"); return <form key={pedido.id} onSubmit={(e) => salvarRastreio(e, pedido.id)}>
        ...
        <details className="admin-customer-details">
          <summary><span><Icon name="document" /></span><b>Dados do comprador e entrega</b><small>CLIQUE PARA VER</small></summary>
        ...
        <div className="admin-email-note"><span><Icon name="document" /></span><p><b>Notificação automática</b><br />Ao salvar, o comprador receberá a nova etapa e o código no e-mail cadastrado.</p></div>
    ...
    <section id="aprovadas-admin">
      ...
      <div className="admin-cards admin-delivery-cards admin-approved-deliveries">{receitasAprovadas.length ? receitasAprovadas.map((receita) => <form key={receita.id} onSubmit={(e) => salvarRastreioReceita(e, receita.id)}>
        ...
        <label>Link para acompanhar<input name="link_rastreio" type="url" defaultValue={receita.link_rastreio || ""} placeholder="https://..." /></label><div className="admin-email-note"><span><Icon name="document" /></span><p><b>Notificação automática</b><br />Ao salvar, o cliente receberá a nova etapa e o rastreamento por e-mail.</p></div><button disabled={salvando === `receita-${receita.id}`}>{salvando === `receita-${receita.id}` ? "SALVANDO..." : "SALVAR E AVISAR CLIENTE"}</button>
    ...
    <section id="historico-admin" className="admin-completed-section"><div className="admin-panel-heading"><div><small>HISTÓRICO</small><h2>Solicitações concluídas</h2><p>Atendimentos finalizados ficam arquivados aqui para consulta.</p></div><span>{solicitacoesConcluidas.length} CONCLUÍDAS</span></div>
      <div className="admin-completed-list">{solicitacoesConcluidas.length ? solicitacoesConcluidas.map((item) => <article key={item.id}><div className="completed-icon"><Icon name="check" /></div><div className="completed-main"><small>PROTOCOLO #{item.id}</small><h3>Pedido {String(item.pedido_id).padStart(6, "0")}</h3><p><b>{item.tipo.replaceAll("_", " ")}</b> • {item.motivo}</p>{item.resposta_admin && <blockquote>{item.resposta_admin}</blockquote>}</div><div className="completed-meta"><strong>CONCLUÍDA</strong><span>{new Date(item.criado_em).toLocaleDateString("pt-BR")}</span></div></article>) : <div className="admin-none">Ainda não existem solicitações concluídas.</div>}</div>
    </section>
  </div>;
}
