  ...
  return (
    <main className="checkout-page">
      ...
      <form onSubmit={finalizar} className="checkout-layout">
        ...
        <aside className="order-summary"><h2>Resumo do pedido</h2>{itens.map((item) => <article key={item.id}><div className="summary-image"><ProductImage nome={item.nome} imagemAtual={item.imagem_url} /></div><div><b>{item.nome}</b><small>Quantidade: {item.quantidade}</small></div><strong>{moeda(Number(item.preco) * item.quantidade)}</strong></article>)}<div className="summary-line"><span>Subtotal</span><b>{moeda(total)}</b></div><div className="summary-line"><span>Frete</span><b>{frete === null ? "A calcular" : frete ? moeda(frete) : "Grátis"}</b></div><div className="summary-total"><span>Total</span><b>{moeda(total + (frete ?? 0))}</b></div><button disabled={processando}>{processando ? (metodoPagamento === "pix" ? "GERANDO PIX..." : "ABRINDO PAGAMENTO...") : (metodoPagamento === "pix" ? "GERAR QR CODE PIX" : "PAGAR COM CARTÃO")}</button><p>{mensagem}</p><small>Pagamento processado com segurança pelo Mercado Pago.</small></aside>
      </form>
    </main>
  );
}
