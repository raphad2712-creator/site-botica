...
export default function CheckoutPage() {
  ...
  async function finalizar(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const resposta = await fetch("/api/pedidos", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin", body: JSON.stringify({ cliente: form, itens: itens.map(({ id, quantidade }) => ({ produto_id: id, quantidade })) }) });
      const dados = await resposta.json();
      ...
      setMensagem("Redirecionando para o ambiente seguro do PagBank...");
  ...
  return (
    <main className="checkout-page">
      <div className="checkout-title"><small>COMPRA SEGURA</small><h1>Finalizar compra</h1><p>Confira os dados antes de seguir para o ambiente seguro do PagBank.</p><div className="checkout-progress" aria-label="Etapas da finalização"><span className="active"><b>1</b>Identificação</span><i /><span className="active"><b>2</b>Entrega</span><i /><span><b>3</b>Pagamento</span></div></div>
      <form onSubmit={finalizar} className="checkout-layout">
        <div className="checkout-forms">
          ...
          <section><header><b>4</b><div><h2>Pagamento</h2><small>Pagamento processado pelo PagBank</small></div></header><div className="pagbank-checkout-choice"><strong>PagSeguro / PagBank</strong><p>Na próxima tela você poderá escolher Pix ou cartão de crédito em até 6x sem juros.</p><div><span>PIX</span><span>VISA</span><span>MASTERCARD</span></div></div><p className="payment-security">Os dados de pagamento são preenchidos diretamente no ambiente seguro do PagBank.</p></section>
        </div>
        <aside className="order-summary"><h2>Resumo do pedido</h2>{itens.map((item) => <article key={item.id}><div className="summary-image"><ProductImage nome={item.nome} imagemAtual={item.imagem_url} /></div><div><b>{item.nome}</b><small>Quantidade: {item.quantidade}</small></div><strong>{moeda(Number(item.preco) * item.quantidade)}</strong></article>)}<div className="summary-line"><span>Subtotal</span><b>{moeda(total)}</b></div><div className="summary-line"><span>Frete</span><b>{cotacaoFrete === null ? "A calcular" : cotacaoFrete.valor ? moeda(cotacaoFrete.valor) : "Grátis"}</b></div><div className="summary-total"><span>Total</span><b>{moeda(total + (cotacaoFrete?.valor ?? 0))}</b></div><button disabled={processando}>{processando ? "ABRINDO PAGBANK..." : "IR PARA O PAGBANK"}</button><p>{mensagem}</p><small>Pagamento processado com segurança pelo PagSeguro/PagBank.</small></aside>
      </form>
    </main>
  );
}
