...
export async function POST(request: Request) {
  ...
  if (admin) await enviarEmail({
    para: admin,
    assunto: `Nova solicitação em análise — pedido #${pedidoId}`,
    html: `<h2>Nova solicitação de pós-venda</h2><p><b>Pedido:</b> #${pedidoId}</p><p><b>Cliente:</b> ${escaparHtml(nome)} (${escaparHtml(auth.user.email)})</p><p><b>Tipo:</b> ${escaparHtml(tipo)}</p><p><b>Motivo:</b> ${escaparHtml(motivo)}</p><p><b>Detalhes:</b> ${escaparHtml(detalhes || "Não informado")}</p><p>Acesse o painel administrativo da Botica para analisar.</p>`,
  });
  if (auth.user.email) await enviarEmail({
    para: auth.user.email,
    assunto: `Recebemos sua solicitação — pedido #${pedidoId}`,
    html: `<h2>Solicitação recebida</h2><p>Olá, ${escaparHtml(nome)}. Sua solicitação referente ao pedido #${pedidoId} foi registrada e está <b>em análise</b>.</p><p>Protocolo: #${solicitacao.id}. A Botica enviará as próximas orientações por e-mail e pela área “Minha conta”.</p>`,
  });
